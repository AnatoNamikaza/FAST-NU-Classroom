// g++ terminal.cpp -o terminal_19L-1196
#include <iostream>
#include<stdlib.h>
#include<stdio.h>
#include<unistd.h>
...
void handle_redirection(string left_command,string file_name,char action);
...
int main(){
  ...
  while(command != "exit"){
	...
	if(contains(left,'<')){
	  ...
	  handle_redirection(left_command,file_name,'r'); // read flag
	}
	  ...
	  handle_redirection(left_command,file_name,'w'); // write flage
	}
	else{
...
void handle_redirection(string command,string file_name,char action){
  int status;
  remove_trailing_spaces(command);
